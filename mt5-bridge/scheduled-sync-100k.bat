@echo off
REM === Velqor Bridge — Sync giornaliero VPS 100K ===
...
if %ERRORLEVEL% NEQ 0 (
    echo [%date% %time%] ERRORE sync 100K >> C:\mt5-bridge\sync_log.txt
) else (
    echo [%date% %time%] Sync 100K OK >> C:\mt5-bridge\sync_log.txt
)
